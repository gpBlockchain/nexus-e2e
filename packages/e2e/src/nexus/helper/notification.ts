...
export async function getSendTransactionMsg(page: Page): Promise<WalletSendTxMsg> {
    ...
    let inputCells = []
    for (let i = 0; i < inputs.match(/\#/g).length; i++) {
        inputCells.push({
            address: (await getByTestId(page, SignMessagePageTestIdInfo.getTransactionInputAddressByIdx(i)).innerText()).split("\n")[1],
            type: await getByTestId(page, SignMessagePageTestIdInfo.getTransactionInputTypeByIdx(i)).innerText(),
            capacity: (await getByTestId(page, SignMessagePageTestIdInfo.getTransactionInputCapacityByIdx(i)).innerText()).replace(" CKB", ""),
        })
    }
    // get output
    const outputs = await getByTestId(page, SignMessagePageTestIdInfo.TransactionOutputs).innerText()
    let outputCells = []
    for (let i = 0; i < outputs.match(/\#/g).length; i++) {
        outputCells.push({
            address: (await getByTestId(page, SignMessagePageTestIdInfo.getTransactionOutputAddressByIdx(i)).innerText()).split("\n")[1],
            type: await getByTestId(page, SignMessagePageTestIdInfo.getTransactionOutputTypeByIdx(i)).innerText(),
            capacity: (await getByTestId(page, SignMessagePageTestIdInfo.getTransactionOutputCapacityByIdx(i)).innerText()).replace(" CKB", ""),
        })
    }
    return {
        inputCells: inputCells, outputCells: outputCells
    }
}
...
async function getCellByHashAndIdx(rpc1: string, txHash, index: string): Promise<Output> {
...
export async function transactionToWalletSendTxMsg(rpc: string, tx: any): Promise<WalletSendTxMsg> {
    ...
    for (let i = 0; i < tx.inputs.length; i++) {
        ...
        inputCells.push({
            address: helpers.encodeToAddress((await getCellByHashAndIdx(rpc, input.previousOutput.txHash, input.previousOutput.index)).lock),
            type: "-",
            capacity: BI.from((await getCellByHashAndIdx(rpc, input.previousOutput.txHash, input.previousOutput.index)).capacity).toNumber().toString()
        })
    }
    return {
